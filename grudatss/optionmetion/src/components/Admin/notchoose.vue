  ...
  <div style="margin-top: 10px">
    <el-table border class="mt-3" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="checkflag" label="状态" align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.checkflag === 2 ? '该学生已被老师拒绝' : '暂无' }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="人物详情" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
  ...
  methods: {
    ...
    deleteItem(item) {
      this.$confirm('是否要删除该选题?', '提示', {
        ...
        this.axios.delete('/admin/admin/delselsub/' + item.selectNO, {
          token: true
        }).then(res => {
          this.getList()
          this.$message({
